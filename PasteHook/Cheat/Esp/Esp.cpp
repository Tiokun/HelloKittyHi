		...
		for (int i = 0; i < cGrenade.GrenadeInfo.size(); i++)
		{
			...
			if (dist < 0.5f)
			{
				if (WorldToScreen(info.vecOrigin, vecOnScreenOrigin))
					g_pRender->DrawWave1(info.vecOrigin, 4, Color::Green());
				...
				if (WorldToScreen((local->GetEyePosition() + vecAngles), vecAngles))
					g_pRender->DrawFillBox(vecAngles.x, vecAngles.y, 7, 7, Color::Blue());

					g_pRender->Text(W / 2 - 200, H / 2 , true, true, Color::White(), (char*)info.szName.c_str());
					g_pRender->Text(W / 2 - 200, H / 2 + 15, true, true, Color::White(), (char*)info.szDescription.c_str());
			}
			else
			{
				if (WorldToScreen(info.vecOrigin, vecOnScreenOrigin));

				g_pRender->DrawWave1(info.vecOrigin, 10, Color::Red());
				g_pRender->DrawWave1(info.vecOrigin, 7, Color::Red());
			}
		}
	}
