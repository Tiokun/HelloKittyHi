	...
	namespace Hook
	{
		...
		bool WINAPI Hook_FireEventClientSideThink(IGameEvent* pEvent)
		{
			if (Settings::Esp::esp_logs)
			{
				...
				if (Settings::Esp::esp_logs)
				{
					ConVar* Developer = Interfaces::GetConVar()->FindVar("developer");
					*(float*)((DWORD)&Developer->fnChangeCallback + 0xC) = NULL;
					Developer->SetValue("1");

					ConVar* con_filter_enable = Interfaces::GetConVar()->FindVar("con_filter_enable");
